...
const Projects = () => {
  ...
  const prevImage = () => {
    setCurrentImage((prevImage) => (prevImage - 1 + projectImages.length) % projectImages.length)
  }

  return (
    <div className="min-h-screen py-12 px-4 md:px-8 lg:px-16 bg-[#D8C7B4] text-gray-300">
      <h1 className="text-center text-4xl font-bold mb-4 text-black">Projects</h1>
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {projectData.map((project, index) => (
          ...
          >
            ...
            />
            <h2 className="text-2xl font-bold mt-4 text-[#f5deb3]">{project.name}</h2>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-black text-gray-300 p-6 rounded-lg max-w-2xl w-full mx-auto">
            <button className="absolute top-2 right-2 text-2xl text-gray-300 hover:text-gray-500" onClick={closeModal}>
              <AiOutlineClose />
            </button>
            <Image
            ...
            <div className="flex justify-between mt-4">
              <button onClick={prevImage} className="text-gray-300 text-2xl hover:text-[#f32170]">
                &lt;
              </button>
              <button onClick={nextImage} className="text-gray-300 text-2xl hover:text-[#f32170]">
                &gt;
              </button>
            </div>
          </div>
